...
@contextmanager
def start_pixiecore(
        server_ip: IPAddress, port: int, ssh_public_key: Path, pxe_image_store_path: Path, hostname: str
    ...
    with TemporaryDirectory(prefix="pixiecore.") as _temp:
        ...
        with uncompressed_initrd_file.open("w+") as f:
            run(["cpio", "-o", "-H" "newc"], cwd=extra_initrd_root, stdout=f, input="./\n./ssh\n./ssh/authorized_keys")
        ...
        with start_pixiecore(
            ...
            try:
                ...
                for event in dnsmasq.next_dhcp_event():
                    print(
                        f"{event.action} client (mac: {event.mac_address}, ip: {event.ip_addr}"
                    )
                    if event.action == "add" or event.action == "old":
                        if event.hostname != random_hostname:
                            print(f"ignore client {event.hostname or event.mac_address} != {random_hostname}")
                            continue
